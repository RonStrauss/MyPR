...
export function HomePage() {
	...
	if (editing) {
		return <PrForm initial={editing} onSubmit={handleEditSubmit} onCancel={() => setEditing(null)} />;
	}

	return (
		<div>
			{celebrationMsg && <CelebrationOverlay message={celebrationMsg} onDone={() => setCelebrationMsg(null)} />}
			...
			{!loading && !error && records.length === 0 && (
				<div className={`${styles.empty} fade-in`}>
					...
					<p>{t('pr.empty')}</p>
					<button type='button' className='btn btn-primary' style={{ marginTop: '1rem' }} onClick={() => navigate('/add')}>
						{t('pr.add')}
					</button>
				</div>
			)}
			...
			<div className={styles.list}>
				{records.map((record, index) => (
					<div key={record.id} className='list-item-enter' style={{ animationDelay: `${index * 50}ms` }}>
						<PrCard record={record} onEdit={setEditing} onDelete={handleDelete} />
					</div>
				))}
			</div>
		</div>
